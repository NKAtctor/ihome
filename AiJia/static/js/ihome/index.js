...
$(document).ready(function(){
    ...
    $.ajax({
        ...
        success: function(data){
            if (data.code == 200){
                $('.register-login').hide();
                $('.user-info .user-name').text(data.user.name);
                $('.user-info').show();
            }
        },
        error: function(data){
                ...
                for (var i = 0; i < 4; i++){
                    $('.swiper-container .swiper-wrapper').append(
                        $('<div>').attr('class', 'swiper-slide').append(
                            $('<a>').attr('href', '/house/detail/?h=' + data.result[i].id).append(
                                $('<img>').attr('src', '/static/media/' + data.result[i].image)
                            ),
                            $('<div>').attr('class', 'slide-title').text(data.result[i].title)
                        )
                    );
                }
